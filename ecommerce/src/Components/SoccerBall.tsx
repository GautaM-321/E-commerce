...
function SoccerBall() {
  ...
  const football_data = data.filter((football_data: itemtype) => {
    if (football_data.category == "football") {
  ...
  function add_to_cart(item: itemtype) {
    if (user_detail.user_status) {
      if (
        ...
        const newwwishlist = Wishlist.filter((product: any) => {
          if (product.id !== item.id) {
            return item;
          }
        });
  ...
  return (
    <>
      ...
      <div className="allproducts">
        {football_data.map((item: itemtype) => {
          return (
